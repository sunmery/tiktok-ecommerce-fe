...
const CategoryNavigator: React.FC<CategoryNavigatorProps> = ({
    ...
    // 初始化：加载根分类的子分类
    useEffect(() => {
        loadCategories(initialRootId).then(() => {
    ...
    const handleCategoryClick = async (category: Category) => {
        ...
        // 如果不是叶子节点，加载其子分类
        loadCategories(category.id).then(() => {
            console.log("loadCategories", categories)
        }).catch(e => {
            console.log("loadCategories error", e)
        })
        loadCategoryPath(category.id).then(() => {
            console.log("loadCategoryPath", breadcrumbs)
        }).catch(e => {
            console.log("loadCategoryPath error", e)
        })
    };
    ...
    const handleGoBack = () => {
        if (breadcrumbs.length > 1) {
            ...
            handleCategoryClick(parentCategory).then(() => {
                console.log("handleCategoryClick", parentCategory)
            }).catch(e => {
                console.log("handleCategoryClick error", e)
            })
        } else {
