...
import { Box, Typography, Grid, Card, CardContent, Modal, Sheet, Button, AspectRatio, CircularProgress } from '@mui/joy';
import { Category } from '@/types/category';
import { categoryService } from '@/api/categoryService';
import { useDirectSubCategories } from '@/hooks/useCategory';
...
const HotCategories: React.FC<HotCategoriesProps> = ({
  ...
  const { 
    ...
    { page: 1, pageSize: 12, status: ProductStatus.APPROVED }
  );
  ...
  const handleSubCategorySelect = (category: Category) => {
    setOpenModal(false);
    navigate({ to: `/products/category/${category.id}` });
  };
