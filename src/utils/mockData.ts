...
// 扩展Product类型，添加quantity和picture属性
type ExtendedProduct = Product & Pick<ProductType, 'quantity' | 'picture'>;
...
export const mockCategoryProducts: Record<string, Product[]> = {
  ...
  'sports-outdoors': mockProducts.filter(p => ['运动户外', '运动鞋'].includes(p.category.categoryName))
};