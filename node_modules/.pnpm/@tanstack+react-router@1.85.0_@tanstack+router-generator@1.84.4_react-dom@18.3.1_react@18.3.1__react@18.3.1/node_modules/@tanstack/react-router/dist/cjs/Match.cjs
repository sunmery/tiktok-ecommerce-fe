...
const MatchInner = React__namespace.memo(function MatchInnerImpl({
  ...
  const { match, matchIndex, routeId } = useRouterState.useRouterState({
    select: (s) => {
      ...
      return {
        ...
        match: utils.pick(match2, ["id", "status", "error"])
      };
    },
  ...
  if (match.status === "error") {
    ...
    if (isServerSideError.isServerSideError(match.error)) {
      const deserializeError = ((_c = router.options.errorSerializer) == null ? void 0 : _c.deserialize) ?? isServerSideError.defaultDeserializeError;
